  ...
  return (
    <>
      <div
        ...
        } max-w-[70vw] lg:w-[400px] lg:min-w-[400px] lg:max-w-[300px] lg:mt-12 p-4 bg-primaryTx max-h-[750px] rounded-md`}
      >
        <h2 className="text-lg font-bold text-primaryBg">
          Hire Dedicated Remote Developers From Brain Inventory
        </h2>
        <span className="flex justify-end absolute top-1 right-4 text-primaryBg cursor-pointer">
          <FaRegWindowMinimize
            onClick={() => {
              localStorage.setItem("openPopup", false);
              setIsOpen(false);
            }}
            className="text-primaryBg cursor-pointer"
          />
        </span>
        <h3 className="text-xs font-semibold flex items-start my-2 text-primaryBg">
          {" "}
          <Image
        ...
        <h2 className="text-xl font-bold my-1 text-primaryBg">Contact Us!</h2>
        <Contact />
      </div>
    </>
  );
